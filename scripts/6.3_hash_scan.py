...
# --- 修改3: calc_hashes 按开关计算 ---
def calc_hashes(image):
    """
    计算 pHash / aHash / dHash / wHash
    返回 16 进制字符串或 None
    """
    phash = str(imagehash.phash(image)) if CALC_PHASH else None
    ahash = str(imagehash.average_hash(image)) if CALC_AHASH else None
    dhash = str(imagehash.dhash(image)) if CALC_DHASH else None
    whash = str(imagehash.whash(image)) if CALC_WHASH else None
